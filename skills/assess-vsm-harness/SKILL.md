...
2. Describe repository architecture before VSM mapping: runtime, agent loop, state, tools, delegation, persistence, scheduling, evaluation, human involvement, maintainer/contributor governance, and external integration when material.
...
9. Record the closure path where the result must affect subsequent regulation or operation. For parent-governed S3/S4/S5, show the relevant matter reaching legitimate parent authority and the returned decision/feedback changing subsequent operation.
...
## Parent-governed ownership

Methodology v0.3.0 allows `P` for S3, S4, and S5. `P` means the function itself is established, but its decisive organizational right is retained by a legitimate parent human, institution, higher recursion, or explicitly evidenced distributed parent arrangement and closes back into subsequent operation.

This is especially relevant to open-source harnesses where multiple independent contributors can operate local agents without a shared pool of private transcripts, runtime state, credentials, or context. Missing centralized access does not imply that every human decision is S5. Current-control decisions may be S3, adaptation decisions may be S4, and identity/ultimate-policy decisions may be S5, each classified by function first and owner second.

A contributor's local intervention does not automatically establish organization-level `P`. The reviewer must reconstruct the function and closure at the declared recursion. If only a lower-recursion contribution cell is evidenced, classify only that boundary.
...
- Methodology v0.3.0 does not extend `P` to S2.
- A manager is not S3; require a whole-system current view plus authority over shared resources, commitments, priorities, constraints, accountability, synergy, or intervention. `S3=P` additionally requires parent ownership and returned closure.
...
- Planning, learning, self-improvement, or event reaction alone is not S4; require an external-and-prospective adaptation loop whose options can affect current capability. `S4=P` additionally requires parent ownership of the decisive adaptation right and return into current capability.
- A prompt, static policy, guardrail, or approval step alone is not S5; require runtime identity or ultimate-policy closure.
- **Generic human approval is not `P`.** The cited human/parent action must close the specific established S3, S4, or S5 function at the declared recursion.
